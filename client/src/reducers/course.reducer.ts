import {
    CREATE_COURSE_FAIL, CREATE_COURSE_REQUEST, CREATE_COURSE_SUCCESS, DELETE_MESSAGE, GET_ALL_INFO_COURSE_FAIL, GET_ALL_INFO_COURSE_REQUEST,
...
import {
    ICourseAction, ICourseInfo, ICourseSummary, ICreateCourseState, IDeleteExerciseType, IExerciseTypeDetail, IMemberSummary, IPaginationInfo,
    IUserCourseState
} from "../interfaces";
...
const courseReducer = (state = initState, action: ICourseAction): IInitState => {
    switch (action.type) {
        ...
        case DELETE_EXERCISE_TYPE_SUCCESS: {
            const idDelete = action.payload as IDeleteExerciseType;

            let newCourse = state.course as ICourseInfo;

            if (state.course && state.course.exerciseTypeList && state.course.exerciseTypeList.length > 0) {
                newCourse.exerciseTypeList = [...state.course.exerciseTypeList];
                newCourse.exerciseTypeList = newCourse.exerciseTypeList.filter(x => x.id !== idDelete.id);
            }
